...
common_parameters = {
    ...
    "sample_period" : 100000,                                               # We use a larger sample period than for the default experiments as sending a single message takes longer here
}
...
for setting in ["noc","pici","aici","wsi"]:
    ...
    for message_size in  [1,4,16,64,256]:
        # MULTI yes or no
        for routing_function in ["modular_routing","multi_routing"]:
            ...
            for mod_routing_function in mod_routing_functions:
                ...
                # It does not make sense to use hashed and adaptive_hashed because this would enforce in-order delivery twice
                    ...
                    hlp.print_error(f"Unknown routing function {routing_function} in experiment_vary_msg_size.py")
                    sys.exit(1)
                # SELECTION FUNCTION
                for mod_selection_function in mod_selection_functions:
                    ...
                    exp['name'] = f"vary_message_size_{setting}_{message_size}_{routing_function}_{mod_routing_function}_{mod_selection_function}"
                    # NOTE: The list of varied parameters must match the order in which they appear in the experiment name (see above)
                    exp['vary'] = ["setting","message_size","routing_function","mod_routing_function","mod_selection_function"]
                    # Settings for plotting, the order in these lists determines the arrangement in the heatmap grid
                    exp['plot_type'] = 'lines'
                    exp['plot_subplot'] = 'setting'
                    exp['plot_x_axis'] = 'message_size'
