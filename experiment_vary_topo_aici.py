...
for traffic in ["uniform","randperm","asymmetric"]:
    # TOPOLOGY
    for topology in ["mesh","double_butterfly","butterdonut","kite_small","kite_large"]:
        ...
        for routing_function in ["modular_routing","multi_routing"]:
            ...
            for mod_routing_function in mod_routing_functions:
                ...
                # It does not make sense to use hashed and adaptive_hashed because this would enforce in-order delivery twice
                    ...
                    hlp.print_error(f"Unknown routing function {routing_function} in experiment_vary_topo_aici.py")
                    sys.exit(1)
                # SELECTION FUNCTION
                for mod_selection_function in mod_selection_functions:
                    ...
                    exp['name'] = f"vary_topo_aici_{traffic}_{topology}_{routing_function}_{mod_routing_function}_{mod_selection_function}"
