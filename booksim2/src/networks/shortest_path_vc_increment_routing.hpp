...
class shortest_path_vc_increment_routing : public routing_function {
public:
    shortest_path_vc_increment_routing() = default;
...
private:
	// routing_table[cur_router_id][dst_node_id] = [port1, port2, ...]
    mutable std::map<int,std::map<int,std::vector<int>>> routing_table;
	// router_list[0][router_id][node_id] = (port, latency)
